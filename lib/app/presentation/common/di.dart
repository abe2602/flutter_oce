...
class ApplicationDI {
  ...
  static MovieListCacheDataSource getMovieListCacheDataSource() =>
      MovieListCacheDataSource();

  static FavoriteMovieListCDS getFavorites() => FavoriteMovieListCDS();

  static MoviesRepository getMoviesRepository(BuildContext context) =>
      MoviesRepository(getMovieDetailRDS(), getMovieRDS(),
          getMovieListCacheDataSource(), context);

  // ignore: close_sinks
  static PublishSubject favoriteDataObservable = PublishSubject<List<Movie>>();

  MovieDetailBloc getMovieDetailBloc(BuildContext context) =>
      MovieDetailBloc(context);

  MovieListBloc getMovieListBloc(BuildContext context) =>
      MovieListBloc(context);

  ReportsBloc getReportsBloc(BuildContext context) =>
      ReportsBloc(context);

  FavoritesBloc getFavoritesBloc(BuildContext context) =>
      FavoritesBloc(context);

  PublishSubject<List<Movie>> getFavoriteDataObservable() =>
      favoriteDataObservable;

  GetMovieListUC getMovieListUC(BuildContext context) =>
      GetMovieListUC(getMoviesRepository(context));

  GetFavoriteListUC getFavoriteListUC(BuildContext context) =>
      GetFavoriteListUC(getMoviesRepository(context));

  GetMovieDetailsUC getMovieDetailsUC(BuildContext context) =>
      GetMovieDetailsUC(getMoviesRepository(context));

  FavoriteMovieUC favoriteMovieUC(BuildContext context) =>
      FavoriteMovieUC(getMoviesRepository(context));
}
